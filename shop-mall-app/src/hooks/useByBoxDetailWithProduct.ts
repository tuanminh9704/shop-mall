...
export const useByBoxDetailWithProduct = (productId: string | null) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (productDetailData?.breadCrumb.length) {
          console.log(productDetailData.breadCrumb);
          setBreadCrumb(productDetailData?.breadCrumb);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const matchVariant = productDetail?.product.productVariant.find(
      (variant) => {
